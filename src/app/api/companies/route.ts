...
import companyService from "@/server/services/companies"
...
export async function GET(req: NextRequest) {
  try {
    ...
    // busca as empresas por usuário
    const data = await companyService.findAllByUserId(session.user.id)
    ...
    console.log("ERROR api/companies/:", error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data = await companyService.create(newCompanie)
